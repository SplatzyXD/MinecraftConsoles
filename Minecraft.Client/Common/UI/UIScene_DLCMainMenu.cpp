...
UIScene_DLCMainMenu::UIScene_DLCMainMenu(int iPad, void* initData, UILayer* parentLayer) : UIScene(iPad, parentLayer)
{
	...
	if (!Windows64_DLCOffers::Get().IsReady())
		Windows64_DLCOffers::Get().FetchFromServer();

	Windows64_DLCOffers::Get().FetchBanners();

	for (int i = 0; i < Windows64_DLCOffers::Get().GetOfferCount(); i++)
	{
		W64_OFFER_INFO& offer = Windows64_DLCOffers::Get().GetOffer(i);

		eDLCContentType eType = e_DLC_MashupPacks;
		...
		CMinecraftApp::RegisterW64DLC(
			eType,
			offer.wszProductID,      // map key
			offer.wszProductID,      // wchBanner / texture name key (same thing)
			i,                       // iConfig
			(unsigned int)i,         // uiSortIndex
			offer.pbBannerData,      // raw PNG bytes (nullptr if fetch failed)
			offer.dwBannerBytes);

		if (offer.pbBannerData != nullptr && offer.dwBannerBytes > 0)
		{
			PBYTE pbCopy = new BYTE[offer.dwBannerBytes];
			memcpy(pbCopy, offer.pbBannerData, offer.dwBannerBytes);
			app.AddMemoryTextureFile(offer.wszProductID, pbCopy, offer.dwBannerBytes);
		}
	}
#endif
